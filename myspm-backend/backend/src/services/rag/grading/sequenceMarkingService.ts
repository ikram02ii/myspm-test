...
import { normalizeAnswerText, studentAnswerCoversIdea, studentExpressesRubricMeaning } from "./gradingFairness";
...
export function sequenceStageAtCorrectPosition(
  ...
  return {
    hit: false,
    evidence: studentAnswer.slice(0, 120),
    reason: `Required stage for position ${positionLabel} not found (expected: ${expectedOrder.join(" â†’ ")}).`,
  };
}
...
export function formatExpectedSequenceForPrompt(rubricIdeas: RubricIdea[]): string {
  return expectedStageOrderFromRubrics(rubricIdeas).join(" â†’ ");
}

/**
 * @deprecated Use sequenceStageAtCorrectPosition â€” kept for call sites.
 */
