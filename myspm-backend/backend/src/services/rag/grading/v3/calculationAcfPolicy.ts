...
/** Generic calculation stages (non-chemistry subjects until dedicated profiles exist). */
export const GENERIC_CALCULATION_STAGE_LABELS = {
  method: "Correct method or approach",
  working: "Correct working",
...
export function inferCalculationPolicy(
  ...
  const domain = subject ? resolveCalculationDomain(subject) : "general";
  if (domain === "chemistry") {
    if (maxScore <= 1) return "answer_only";
    return "show_working";
  }
  return questionRequiresShownWorking(question) ? "show_working" : "answer_only";
}
...
export function showWorkingStagePlan(
  ...
  if (maxScore === 2) {
    return [
      { label: GENERIC_CALCULATION_STAGE_LABELS.method, weight: 1 },
  ...
  return [
    { label: GENERIC_CALCULATION_STAGE_LABELS.method, weight: 1 },
    { label: GENERIC_CALCULATION_STAGE_LABELS.working, weight: 1 },
