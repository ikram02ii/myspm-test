...
export function scoreCalculationDemonstration(
  ...
  // Implied-formula rule: correct substitution/working can only be produced by
  // applying the formula/method, so the first (formula/method) stage is earned
  // whenever any intermediate working stage is demonstrated — even if the bare
  // formula line is not written out separately. This mirrors SPM examiner
  // practice (a correctly substituted line earns the formula mark) and prevents
  // a spurious 1-mark deduction when only the standalone formula line is absent.
  // The final-answer stage is excluded, so a final-only answer earns no bonus.
  if (ordered.length >= 3) {
