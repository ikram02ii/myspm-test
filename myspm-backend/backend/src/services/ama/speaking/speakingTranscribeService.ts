...
async function qwenTranscribeFromBuffer(
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (taskStatus === "FAILED") {
      const failMsg = status.output?.message ?? JSON.stringify(status.output);
      if (
        failMsg.includes("SUCCESS_WITH_NO_VALID_FRAGMENT") ||
        failMsg.includes("NO_VALID_FRAGMENT") ||
        failMsg.includes("DECODE_ERROR")
      ) {
        console.warn("[speaking] transcription soft-fail, using empty transcript:", failMsg);
        return "";
      }
      throw new Error(`Qwen transcription failed: ${failMsg}`);
    }
  }
