...
import { pastPaperFormWhereClause, textbookFormWhereClause, normalizeForm, FULL_SYLLABUS_FORMS } from "./pastPaperFormFilter";
import type {
...
export type RetrieveVlTbChunksInput = RetrieveChunksInput & {
  /** `rag_tb.tb_id` — e.g. tb-vl-bio-f4 */
  tbId: string;
};

/** Textbook chunk retrieval from vision-ingested `rag_tb` / `rag_tb_chunks` (same scoring as legacy). */
export async function retrieveVlTbChunks(input: RetrieveVlTbChunksInput): Promise<RetrieveChunksResult> {
  const query = input.query.trim();
  if (!query) throw new Error("query is required");
  if (!ragDb) throw new Error("RAG database is not configured");

  const tbId = input.tbId.trim();
  if (!tbId) throw new Error("tbId is required");
  ...
  const chapterFilterSanitized =
    chapterFilterRaw && chapterFilterRaw.length >= 2 ? sanitizeChapterFilter(chapterFilterRaw) : "";
  const chapterFilterClause =
    chapterFilterSanitized.length >= 2 ? ilike(ragTbChunksTable.chapter, `%${chapterFilterSanitized}%`) : undefined;
  const chapterHintLower = input.chapterHint?.trim().toLowerCase() ?? "";
  const requestedTopK = typeof input.topK === "number" ? input.topK : Number.NaN;
  const topK = Number.isFinite(requestedTopK) ? Math.max(1, Math.min(20, Math.floor(requestedTopK))) : 6;
  const candidateLimit = Math.min(500, Math.max(100, topK * 20));
  ...
  const equationAnchors = extractEquationAnchors(query);
  const conceptProfile = getConceptProfile(query);
  if (queryTokens.length === 0) return { query, count: 0, chunks: [] };

  const tokenPredicates = searchTokens.flatMap((token) => [
    ilike(ragTbChunksTable.content, `%${token}%`),
    ilike(ragTbChunksTable.conceptTitle, `%${token}%`),
    ilike(ragTbChunksTable.conceptSummary, `%${token}%`),
    ilike(ragTbChunksTable.keywords, `%${token}%`),
    ilike(ragTbChunksTable.figures, `%${token}%`),
    ilike(ragTbChunksTable.tables, `%${token}%`),
  ]);
  const tokenClause = or(...tokenPredicates);
  if (!tokenClause) return { query, count: 0, chunks: [] };

  const qForm = normalizeForm(form);
  const vlFormClause =
    qForm && qForm.toLowerCase() !== "general"
      ? or(eq(ragTbTable.form, qForm), inArray(ragTbTable.form, [...FULL_SYLLABUS_FORMS] as string[]))
      : undefined;

  const filters = [
    eq(ragTbTable.tbId, tbId),
    subject ? eq(ragTbTable.subject, subject) : undefined,
    vlFormClause,
    chapterFilterClause,
  ].filter((v): v is NonNullable<typeof v> => v != null);
  const whereClause = and(...filters, tokenClause);

  const rows = await ragDb
    .select({
      textbookId: ragTbTable.tbId,
      subject: ragTbTable.subject,
      form: ragTbTable.form,
      title: ragTbTable.title,
    ...
    .innerJoin(ragTbTable, eq(ragTbChunksTable.tbDbId, ragTbTable.id))
    .where(whereClause)
    .limit(candidateLimit);

  const qualityFilteredRows = rows.filter(
    (row) => !isLowQualityChunk(row.content) && !row.content.includes("[MODERATION_BLOCKED]"),
  );

  const textbookScored: RetrievedChunk[] = qualityFilteredRows.map((row) => {
    const chapterBoost =
      chapterHintLower.length >= 2 && row.chapter?.toLowerCase().includes(chapterHintLower) ? 0.45 : 0;
    const retrievalText = [row.conceptTitle ?? "", row.conceptSummary ?? "", row.keywords ?? "", row.content]
      .filter(Boolean)
      .join("\n");
    const base =
      conceptProfile == null
    ...
    return {
      ...
      keywords: row.keywords ? row.keywords.split(",").map((k) => k.trim()).filter(Boolean) : undefined,
      chapter: row.chapter ?? undefined,
      pageStart: row.pageStart ?? undefined,
      pageEnd: row.pageEnd ?? undefined,
      content: row.content,
  ...
  return { query, count: scored.length, chunks: scored };
}
...
function orderChapterBuckets<T extends { chapter?: string | null }>(
