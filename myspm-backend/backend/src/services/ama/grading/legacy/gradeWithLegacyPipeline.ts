/**
 * Legacy (v1) single-shot LLM grading pipeline.
 * Kept for RAG_GRADE_PIPELINE=v1|legacy|off|false|0. Behaviour preserved.
 */
import type {
  DiagramContext,
  MarkBreakdownItem,
 ...
 * student gets the feature mark only â€” never the unearned function mark.
 */
function enforceFeatureFunctionRule(parsed: QwenGradeShape, context: {
  ...
  for (const row of parsed.markBreakdown) {
    ...
    if (row.awarded && split && row.marks >= 2) {
      ...
      const functionIdea = `${split.feature} â†’ ${split.functionPart}`;
      newBreakdown.push({
        idea: functionIdea,
        awarded: false,
        marks: functionMarks,
  ...
  const newFeedback = sanitizeFeedback(
    "You gave the right parts, but you did not say what each part does. Add a short link for each part â€” for example say what it helps or why it matters (use 'because', 'so that', or 'to ...') â€” to get the rest of the marks.",
  );
...
function sanitizeFeedback(raw: string, opts?: { maxSentences?: number }): string {
  return sanitizeLearnerFeedback(raw, opts);
}
...
function detectAnswerStyle(answer: string): AnswerStyle {
  ...
  const hasCalculation =
    /[\d.]+\s*(?:[+\-*/xÃ—Ã·=]|mol|g\b|kg\b|cm\b|mm\b|ml\b|kj\b|kJ\b)/i.test(text) ||
    /=\s*[\d.]+/.test(text);
  ...
  // Short list-like answer with no causal words â†’ stating (e.g., "Thin walls and many capillaries.")
...
function buildQuestionTypeRules(type: QuestionType): string {
  ...
  switch (type) {
    ...
    case "identify":
      return [
        header,
        "- Mark by CONCEPT, not model-answer wording: award when the required term/keyword/process/structure is present.",
        "- Do NOT require a full sentence, explanation, or 'because' / 'kerana' unless the question explicitly asks.",
        "- A single correct keyword or short phrase is enough for full marks when it shows the required concept.",
        "- If the stem already states background facts, that text is GIVEN â€” grade only what the student must supply.",
        "- Accept BM/EN synonyms and common classroom paraphrases when the scientific concept is clear.",
        "- If correct: confirm briefly. If missing: list ONLY missing items. If wrong: give correct term(s) in one short line.",
      ].join("\n");
    case "define":
      return [
        header,
    ...
    case "explain":
      return [
        header,
        "- Award each correct scientific idea independently; accept paraphrases â€” not textbook wording only.",
        "- SPM Form 4/5 depth only â€” never A-Level / university depth.",
        "- For an N-mark Explain, expect ~N short mark points (cause/effect at SPM level).",
        "- Ignore grammar/spelling if meaning is clear. Do not require examples unless the question asks.",
        "- Model answer = reference concepts only; alternative correct explanations earn the same marks.",
    ...
    case "describe":
      return [
        header,
        "- Award each correct descriptive point independently; accept student-friendly paraphrases.",
        "- SPM-level descriptive points only, in the order an examiner expects when order matters.",
        "- For an N-mark Describe, expect ~N short ordered points.",
        "- Do not require examples unless asked. Model answer is concept reference only.",
    ...
    case "diagram_label":
      return [
        header,
        "- The structured diagram block is for rubric context only â€” NOT proof the student knows a label.",
        ...
        "- Do NOT award explanation/function marks here â€” labelling questions test recognition in the written answer only.",
        ...
        "- Feedback: short confirmation (e.g., 'P = phloem âœ“, Q = xylem âœ“.'). For misses, give the correct labels in one short line.",
      ].join("\n");
    case "graph_reading":
      return [
        header,
        "- Use axes/dataPoints/keyValues from the figure only to know what values/trends are expected â€” not as student evidence.",
        "- Award a read-off, gradient, intercept, or trend mark ONLY if the student stated that value/trend in their written answer.",
        "- For 'read off' questions, accept values within Â±1 small grid square or Â±5% of the expected value when the student wrote a number.",
...
async function gradeWithQwen(params: {
  ...
  const featureFunctionGradingParagraph = featureFunctionMode
    ? [
        "Feature + function rule (APPLIES â€” explain/discuss, or describe-with-adaptation/function):",
        ...
        "    * Withhold: 'thin walls â†’ reduces diffusion distance' (function) = 0m, 'many capillaries â†’ maintain steep diffusion gradient' (function) = 0m.",
    ...
    : [
        ...
        "- Do NOT force paired feature+function rows for pure structure description (e.g. 'Describe the structure of â€¦') unless the stem asks for adaptation, role, effect, how, or why.",
  ...
  const systemPrompt = [
    "You are a strict, rubric-based SPM (Form 4/5) examiner for Malaysian students. You MUST follow every rule below exactly.",
    formatSpmStudentFriendlyRulesBlock(),
    "You MUST return JSON ONLY (no prose, no code fences) with these fields:",
    "{",
  ...
  const gradingRules = [
    ...
    "- 1) Identify the command word and what it requires (State/Name = term or keyword; Explain/Describe = scientific ideas; Compare = paired differences; etc.).",
    "- 2) Build markBreakdown: each rubric/mark-scheme CONCEPT (idea), whether the student showed that concept (awarded), marks, and a brief reason.",
    "     For each row with awarded=true: reason MUST quote the exact student phrase that earned the mark, then link it to the rubric point.",
    "     For awarded=false: reason must say the required point was not found in the student's written answer (no implied credit).",
    `- 3) Compute score = sum of marks where awarded === true. score MUST equal that sum and MUST NOT exceed maxScore (${params.maxScore}).`,
    "- 4) Fill matchedIdeas from concepts awarded; missingIdeas from concepts not shown (short labels, not model-answer prose).",
    "- 5) Write feedback from matched + missing concepts only — brief, SPM Form 4/5 school language (not A-Level/university).",
    buildFeedbackVerbFormatRulesBlock(params.maxScore, params.question),
    "- 6) Write modelAnswer at EXACTLY SPM Form 4/5 KSSM level — not A-Level, not STPM, not university, not advanced.",
    ...
    "     NEVER require students to copy it verbatim — it is a concept reference, not the only correct wording.",

    "Question vs answer-style alignment (CRITICAL â€” prevents over-marking):",
    ...
    "Calibration:",
    "- CRITICAL EVIDENCE: quote student wording â†’ match rubric point â†’ award only if the quote exists; never infer; never treat model answer as student evidence.",
    "- Mark by CONCEPT presence in the student answer, not similarity to model-answer wording.",
    "- Award marks ONLY for ideas explicitly stated or clearly conveyed â€” never inferred.",
    "- Reject vague or generic lines that could fit many topics even if scientifically related.",
    "- Grade at SPM Form 4/5 level â€” NOT A-Level / matriculation / university.",
    "- Accept paraphrases and short keywords when they show the required concept (especially State/Name questions).",
    ...
    "- '[PAST PAPER MARK SCHEME]' is NOT the rubric for this exact question â€” it shows SPM answer style, mark splits, accepted phrasings, and strictness ceiling.",
    ...
    "FEEDBACK STYLE (mandatory SPM examiner voice):",
    "- You MUST follow STUDENT LANGUAGE LEVEL in your system instructions for feedback, modelAnswer, strengths, improvements, and markBreakdown reasons.",
    "- Length: 1–3 short sentences ONLY. NEVER use paragraphs or padding.",
    "- Base feedback ONLY on matchedIdeas + missingIdeas; do not introduce new ideas not in the breakdown.",
    "- NEVER criticize the student for missing an idea that is not an explicit individual marking point in the rubric/markBreakdown.",
    ...
    "- Partial marks: one sentence on what was correct, then name the TYPE of missing point — do NOT write the full correct answer.",
    "- Zero/low marks: say it's too vague or incorrect; name what kind of point was missing — do NOT give the model answer in feedback.",
    ...
    "- NEVER include 'Model answer:' inside feedback â€” modelAnswer is a separate field.",
  ...
  const fairnessBlock = [
    ...
    "- Retrieved context is evidence only â€” never invent mark rows purely because a chunk mentions a fact the question did not ask for.",
    "CONTRADICTION CHECK:",
    "- missingIdeas MUST NOT contain any point that the student already wrote, including paraphrases (e.g. grow/form/develop pollen tube; reaction rate â‰ˆ speed of reaction; provides energy â‰ˆ gives energy).",
  ...
  const mcqLetterExplain = isMcqLetterOnlyExplanationRequest(
    ...
    ? [
        "SPECIAL CASE â€” SPM OBJECTIVE (MCQ): The student answer is ONLY one letter Aâ€“D.",
        ...
        "FEEDBACK (override generic 1â€“3 sentence brevity for this case only), plain text inside the JSON string, no bullets:",
        "  If the student's letter is WRONG: write 4â€“6 short sentences in simple words Form 4/5 can follow. (1) State clearly that their letter is incorrect. (2) Explain WHY that chosen option fails the stem (wrong fact, irrelevant, or misread). (3) State the CORRECT letter and quote a key phrase from that option line. (4) Explain WHY the correct option answers the question.",
        "  If the student's letter is RIGHT: write 2â€“3 short simple sentences confirming correctness and why that option fits the stem.",
        ...
        "modelAnswer: one line â€” correct letter, closing parenthesis, then a brief reason.",
...
export {
  gradeWithQwen as gradeWithLegacyPipeline,
