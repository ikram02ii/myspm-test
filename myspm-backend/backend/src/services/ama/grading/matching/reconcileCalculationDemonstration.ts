...
import { quoteGroundedInStudentAnswer } from "./udmEvidenceGate";
import type { AssessmentCaseFile, UnderstandingDemonstration } from "../shared/types";
import { isUdmTraceEnabled } from "../shared/udmTickTrace";

function logCalcStageDecision(payload: Record<string, unknown>): void {
  if (!isUdmTraceEnabled() && process.env.GRADE_CALC_TRACE !== "1") return;
  console.info("[grade:calcTrace]", payload);
}
...
export function reconcileCalculationDemonstration(params: {
  ...
  // --- Ground every credited stage in the student answer (no hallucinated quotes) ---
  // The calc evaluator does not run the theory evidence gate, so the LLM can invent
  // formula/working lines from the model answer. Those must never earn marks.
  {
    const groundedDemos = udm.unitsDemonstrated.map((d) => {
      if (!d.valid) return d;
      ...
      if (!quote) {
        ...
        if (!isFinal) {
          logCalcStageDecision({
            decision: "deduct",
            unitId: d.unitId,
            reason: "empty_quote_non_final",
          });
          return { ...d, valid: false };
        }
        return d;
      }
      if (!quoteGroundedInStudentAnswer(quote, params.studentAnswer)) {
        logCalcStageDecision({
          decision: "deduct",
          unitId: d.unitId,
          reason: "quote_not_in_student_answer",
    ...
    udm = { ...udm, unitsDemonstrated: groundedDemos };
  }
  ...
  if (showWorking && substitutionId) {
    ...
    if (
      subDemo &&
      !quoteLooksLikeSubstitution(subDemo.quote) &&
      !quoteLooksLikeSubstitution(params.studentAnswer)
    ) {
      udm = invalidateUnit(
        udm,
        substitutionId,
      ...
      logCalcStageDecision({
        decision: "deduct",
        unitId: substitutionId,
        reason: "missing_substitution_shape",
      });
    }
  }
  ...
  if (showWorking && calculationId) {
    ...
    if (
      calcDemo &&
      !quoteLooksLikeSubstitution(calcDemo.quote) &&
      !quoteLooksLikeSubstitution(params.studentAnswer)
    ) {
      udm = invalidateUnit(
        udm,
      ...
      logCalcStageDecision({
        decision: "deduct",
        unitId: calculationId,
        reason: "missing_calculation_shape",
      });
    }
  }
