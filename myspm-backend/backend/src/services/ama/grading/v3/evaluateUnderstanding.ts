...
export async function evaluateUnderstanding(params: {
  ...
  const system = [
    ...
    "COMPOUND SENTENCES: one student sentence may demonstrate multiple units — credit each unit separately and quote the same sentence when it contains both mark points.",
    "PARTIAL CREDIT (max 2 marks): award 1 mark when the answer is scientifically correct but incomplete; reserve 0 only for wrong, irrelevant, or empty answers.",
    "VAGUE ANSWERS: generic phrases ('one is changed', 'digests proteins') without the required scientific term do NOT earn full marks — credit only units explicitly supported.",
    "COMPARE/DIFFERENCE stems: both sides must be named or clearly identified — 'one...the other' without the scientific terms earns at most ONE unit, not full marks.",
