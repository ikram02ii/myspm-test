...
} from "../perPointModelAnswer";
import { splitModelAnswerIntoPoints } from "../splitModelAnswerPoints";
import type {
  AssessmentCaseFile,
...
export type OpenEndedAgentScoreBundle = {
  ...
  chainWalk?: import("../types").GradingContext["chainWalk"];
};
...
export async function presentOpenEndedAgentResult(params: {
  ...
  const perPointExemplars = !isCalc
    ? resolvePerPointExemplars({
        acf: ctx.acf,
        question: ctx.question,
  ...
  const localized =
    ctx.officialMa.text && score < ctx.maxScore
  ...
  if (!isCalc && modelAnswer) {
    ...
    // Rewrite when the exemplar still looks like one blob despite a multi-mark scheme
    // (e.g. inline "• a • b • c" used to skip the old includes("•") collapse guard).
  ...
  const modelAnswerPointCards = !isCalc
    ? (() => {
        // Structured cards from assessment units — root source of truth for UI.
        if (perPointExemplars.length > 0) {
          const expected = Math.max(ctx.maxScore, perPointExemplars.length);
          let texts = perPointExemplarTexts;
          if (modelAnswer) {
            const fromLocalized = splitModelAnswerIntoPoints(modelAnswer, expected);
            // Prefer localized polish only when it yields exactly one block per unit.
            if (fromLocalized.length === perPointExemplars.length) {
              texts = fromLocalized;
            }
          }
          return perPointExemplars.map((p, i) => ({
            text: (texts[i] || p.exemplar || p.rubricLabel).trim(),
            marks: Math.max(1, p.marks || 1),
          }));
        }
        if (modelAnswer) {
          const expected = Math.max(markSchemePoints.length, ctx.maxScore);
          return splitModelAnswerIntoPoints(modelAnswer, expected).map((text) => ({
            text,
            marks: 1,
          }));
        }
  ...
  const modelAnswerPoints =
    modelAnswerPointCards && modelAnswerPointCards.length > 0
      ? modelAnswerPointCards.map((c) => c.text)
      : undefined;
