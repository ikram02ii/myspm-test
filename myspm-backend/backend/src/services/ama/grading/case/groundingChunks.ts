...
import { retrieveChunks, retrieveVlTbChunks, isLowQualityChunk } from "../../retrieval/retrievalService";
...
async function retrieveTextbookChunks(params: {
  ...
  chapterHint?: string;
  /** When set, search vision-ingested `rag_tb_chunks` instead of legacy `rag_textbook_chunks`. */
  vlTbId?: string;
}): Promise<RetrievedChunk[]> {
  const result = params.vlTbId
    ? await retrieveVlTbChunks({
...
export type RetrievedEvidenceContext = {
  ...
  /** Textbook chunks selected for rubric grounding (with relevance scores). */
  textbookChunks: RetrievedChunk[];
};
...
export async function retrieveEvidenceContext(params: {
  ...
  chapterHint?: string;
  /** When set, use vision-ingested textbook (`rag_tb`) instead of legacy chunks. */
  vlTbId?: string;
  ...
  const search = async (query: string): Promise<RetrievedChunk[]> => {
    const base = {
      ...
      form: params.form,
      chapterHint,
      vlTbId: params.vlTbId,
    };
    if (chapterFilter) {
      const scoped = await retrieveTextbookChunks({ ...base, chapterFilter });
      if (scoped.length > 0) return scoped;
    }
    return retrieveTextbookChunks(base);
  };
